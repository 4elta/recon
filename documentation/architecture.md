...
An analyzer package consists of an analyzer (i.e. `__init__.py`) and at least one parser (i.e. `parsers/<tool>.py`; one per tool).
...
These recommendations are specified as TOML files (i.e. `recommentations/*.toml`).
...
The issue descriptions (e.g. "protocol supported: TLS 1.0"), recommendations (e.g. "disable support for TLS 1.0") and links to references are stored in a TOML file (i.e. `issues/<language>.toml`).
