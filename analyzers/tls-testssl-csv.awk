#!/usr/bin/awk -f
# analyze testssl result files; output CSV
...
/^ SSLv2 + offered/ {
  printf "%s,%s\n", service, desc_protocol_ssl2
  next
}

/^ SSLv3 + offered/ {
  printf "%s,%s\n", service, desc_protocol_ssl3
  next
}

/^ TLS 1 + offered/ {
  printf "%s,%s\n", service, desc_protocol_tls10
  next
}

/^ TLS 1.1 + offered/ {
  printf "%s,%s\n", service, desc_protocol_tls11
  next
}
...
/^ Signature Algorithm/ && ! /SHA(256|384|512)/ {
  ...
  printf "%s,%s:", service, desc_cert_sig_alg
...
/^ Server key size/ && /RSA/ {
  ...
  if ($5 < 3000) {
    printf "%s,%s: %s\n", service, desc_cert_short_rsa_key, $5
  }
  next
}
...
/^ Chain of trust/ && ! /Ok/ {
  ...
  printf "%s,%s:", service, desc_cert_trust
...
/Certificate Validity/ && /expired/ {
  ...
  printf "%s,%s %s %s -- %s %s\n", service, desc_cert_expired, $5, $6, $8, $9
  next
}
...
/^ LUCKY13 \(CVE-2013-0169\)/ && ! /not vulnerable/ {
  printf "%s,%s\n", service, desc_vuln_lucky13
  next
}

# https://heartbleed.com/
/^ Heartbleed \(CVE-2014-0160\)/ && ! /not vulnerable/ {
  printf "%s,%s\n", service, desc_vuln_heartbleed
  next
}
...
/^ CCS \(CVE-2014-0224\)/ && ! /not vulnerable/ {
  printf "%s,%s\n", service, desc_vuln_ccs
  next
}

# https://freakattack.com/
/^ FREAK \(CVE-2015-0204\)/ && ! /not vulnerable/ {
  printf "%s,%s\n", service, desc_vuln_freak
  next
}
